...
if (isProd) {
  ...
  cdn = {
    ...
    js: [
      'https://v2.cn.vuejs.org/js/vue.min.js', // vue.js
...
module.exports = {
  ...
  configureWebpack: {
    ...
    // 要排除的包名
    // key(是要排除的包名)：value(实际上是实际引入的包的变量名 )
    // 因为要排除element-ui所以后面要引入cdn文件 CDN文件中有ELEMENT的全局变量名
    // externals首先会被排除 定义的包名，空出来的位置，会用变量来替换
    externals: externals
  },
  chainWebpack(config) {
    ...
    // 注入CDN变量
    // 这行代码会在执行打包的时候 执行就会将cdn变量注入到 html模板中
