...
export default {
  ...
  methods: {
    ...
    async saveUser() {
      ...
      // upload如果为true，表示该图片已经完成上传，以此来判断图片是否已经上传完成
      if (fileList.some((item) => !item.upload)) {
        // 如果if括号内的条件是成立的，就意味着有图片没有上传完成
        this.$message.warning('您当前还有图片没有上传完成！')
        return // 函数中碰到return 函数就停止执行
      }
      // 用户点击了保存更新，调用接口，并且把用户输入的内容传递过去
